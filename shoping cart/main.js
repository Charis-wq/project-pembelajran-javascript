class Product{
    ...
    displayCard(){
        ...
        <button onclick="shoppingCart.viewProductDetail(${this.id})">View Detail</button>
...
class ShoppingCart{
    ...
    viewProductDetail(id){
        ...
        document.getElementById('detai-image').src = product.image
        document.getElementById('detail-tittle').textContent = product.name
    ...
    toogleProductDetail(addFromDisplay = false){
        const modal = document.getElementById('product-modal')
        modal.style.display = modal.style.display === 'block' ? 'none' : "block"
    }
    addToCart(id){
        const product = this.products.find(p => p.id === id)
        const exitingItem = this.cart.find(item => item.product.id === id)

        if(exitingItem){
            exitingItem.incertmentQuantity()
        }else {
    ...
    removeFromCart(index){
        const removeItem = this.cart.splice(index,1)[0]
        this.updateCart()
        this.updateProductQuantity(removeItem.product.id)
    }
    toogleCart(){
...
shoppingCart.addProduct(
    new Product(
		...
		20.0,
        'https://images.unsplash.com/photo-1618366712010-f4ae9c647dcb?q=80&w=1376&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
    )
);
shoppingCart.addProduct(
    new Product(
		...
		30.0,
        'https://images.unsplash.com/photo-1531104985437-603d6490e6d4?q=80&w=1439&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
    )
);
